// flow-typed signature: 941c2a283a18085d92480b12a9e4c28d
// flow-typed version: 3c17c8ecd5/@react-navigation/material-top-tabs_v5.x.x/flow_>=v0.104.x
...
declare module '@react-navigation/material-top-tabs' {
	...
	declare type $If<Test: boolean, Then, Else = empty> = $Call<
		...
		Else
	>;
	declare type $IsA<X, Y> = $Call<((Y) => true) & ((mixed) => false), X>;
	...
	declare export type SetParamsAction = {|
		...
		+target?: string
	|};
	declare export type CommonAction = BackAction | NavigateAction | ResetAction | SetParamsAction;
	...
	declare export type PossiblyStaleNavigationState = NavigationState | StaleNavigationState;
	...
	declare type ActionCreators<State: NavigationState, Action: GenericNavigationAction> = {
	...
	declare export type Router<State: NavigationState, Action: GenericNavigationAction> = {|
	...
	declare export type JumpToAction = {|
		...
		+target?: string
	|};
	declare export type TabAction = CommonAction | JumpToAction;
	...
	declare type DrawerHistoryEntry = {| +type: 'route', +key: string |} | {| +type: 'drawer' |};
	...
	declare type EventPreventDefaultProperties<Test: boolean> = $If<
		...
		{||}
	>;
	declare type EventDataProperties<Data> = $If<$IsUndefined<Data>, {||}, {| +data: Data |}>;
	declare type EventArg<EventName: string, CanPreventDefault: ?boolean = false, Data = void> = {|
	...
	> = (
		e: EventArg<
			EventName,
			$PropertyType<
				$ElementType<{| ...EventMap, ...EventMapCore<State> |}, EventName>,
				'canPreventDefault'
			>,
			$PropertyType<
				$ElementType<{| ...EventMap, ...EventMapCore<State> |}, EventName>,
	...
	declare export type SimpleNavigate<ParamList> = <DestinationRouteName: $Keys<ParamList>>(
		routeName: DestinationRouteName,
		params: $ElementType<ParamList, DestinationRouteName>
	) => void;

	declare export type Navigate<ParamList> = SimpleNavigate<ParamList> &
	...
	> = {
		...
		+dispatch: (action: GenericNavigationAction | ((State) => GenericNavigationAction)) => void,
		...
		+addListener: <EventName: $Keys<{| ...EventMap, ...EventMapCore<State> |}>>(
			name: EventName,
			callback: EventListenerCallback<EventName, State, EventMap>
		) => () => void,
		+removeListener: <EventName: $Keys<{| ...EventMap, ...EventMapCore<State> |}>>(
			name: EventName,
			callback: EventListenerCallback<EventName, State, EventMap>
		) => void,
		...
	...
	declare export type NavigationProp<
		...
		EventMap: EventMapBase = EventMapCore<State>
	> = {
		...$Exact<NavigationHelpers<ParamList, State, EventMap>>,
	...
	declare export type RouteProp<ParamList: ParamListBase, RouteName: $Keys<ParamList>> = {|
		...LeafRoute<RouteName>,
	...
	> =
		| {|
				...BaseScreenProps<ParamList, NavProp, RouteName, State, ScreenOptions, EventMap>,
	...
	> = <
		...
		NavProp: NavigationProp<GlobalParamList, RouteName, State, ScreenOptions, EventMap>
	>(
		props: ScreenProps<ParamList, NavProp, RouteName, State, ScreenOptions, EventMap>
	) => React$Node;
	...
	> = <
		GlobalParamList: ParamListBase,
		ParamList: ParamListBase,
		NavProp: NavigationHelpers<GlobalParamList, State, EventMap>
	>() => {|
		+Screen: ScreenComponent<GlobalParamList, ParamList, State, ScreenOptions, EventMap>,
	...
	declare export type CreateNavigatorFactory = <
		...
		ScreenOptions: { ... },
		EventMap: EventMapBase,
		NavProp: NavigationHelpers<ParamListBase, State, EventMap>,
		ExtraNavigatorProps: ExtraNavigatorPropsBase
	>(
		navigator: React$ComponentType<{|
			...$Exact<ExtraNavigatorPropsBase>,
	...
	declare export type Descriptor<NavProp, ScreenOptions: { ... } = { ... }> = {|
		+render: () => React$Node,
	...
	declare export type TransitionSpec =
		| {|
				animation: 'spring',
				config: $Diff<SpringAnimationConfigSingle, { toValue: number | AnimatedValue, ... }>
		  |}
		| {|
				animation: 'timing',
				config: $Diff<TimingAnimationConfigSingle, { toValue: number | AnimatedValue, ... }>
		  |};
	...
	declare export type StackDescriptor = Descriptor<StackNavigationProp<>, StackOptions>;
	...
	declare export type StackHeaderTitleInputProps = $Exact<StackHeaderTitleInputBase>;
	...
	> = {
		...$Exact<NavigationProp<ParamList, RouteName, StackNavigationState, Options, EventMap>>,
	...
	declare export type BottomTabBarButtonProps = {|
		...$Diff<TouchableWithoutFeedbackProps, {| onPress?: ?(event: PressEvent) => mixed |}>,
		+to?: string,
		+children: React$Node,
	...
	declare export type BottomTabOptions = $Shape<{|
		+title: string,
		+tabBarLabel: string | (({| focused: boolean, color: string |}) => React$Node),
		+tabBarIcon: ({|
			focused: boolean,
			color: string,
			size: number
	...
	declare type InexactTabNavigationProp<
		...
		EventMap: EventMapBase
	> = {
		...$Exact<NavigationProp<ParamList, RouteName, TabNavigationState, Options, EventMap>>,
	...
	declare export type MaterialBottomTabOptions = $Shape<{|
		...
		+tabBarColor: string,
		+tabBarLabel: string,
		+tabBarIcon: string | (({| +focused: boolean, +color: string |}) => React$Node),
	...
	declare export type MaterialBottomTabNavigatorProps<
		NavProp: InexactMaterialBottomTabNavigationProp<> = MaterialBottomTabNavigationProp<>
	> = {|
	...
	declare export type MaterialTopTabNavigatorProps<
		NavProp: InexactMaterialTopTabNavigationProp<> = MaterialTopTabNavigationProp<>
	> = {|
	...
	declare export type DrawerOptions = $Shape<{|
		title: string,
		drawerLabel: string | (({| +color: string, +focused: boolean |}) => React$Node),
		drawerIcon: ({|
			+color: string,
			+size: number,
			+focused: boolean
	...
	> = {
		...$Exact<NavigationProp<ParamList, RouteName, DrawerNavigationState, Options, EventMap>>,
		+jumpTo: SimpleNavigate<ParamList>,
		+openDrawer: () => void,
		+closeDrawer: () => void,
		+toggleDrawer: () => void,
	...
	declare export type DrawerDescriptor = Descriptor<DrawerNavigationProp<>, DrawerOptions>;
	...
	declare export type BaseNavigationContainerInterface = {|
		...$Exact<
			NavigationHelpers<ParamListBase, PossiblyStaleNavigationState, ContainerEventMap>
		>,
	...
	declare export type GetFocusedRouteNameFromRoute = (PossiblyStaleRoute<string>) => ?string;
	...
	declare export type MaterialTopTabViewProps = {|
		...
		...MaterialTopTabNavigationBuilderResult
	|};
	declare export var MaterialTopTabView: React$ComponentType<MaterialTopTabViewProps>;

	/**
	 * MaterialTopTabBar
	 */

	declare export var MaterialTopTabBar: React$ComponentType<MaterialTopTabBarProps>;
}
